function _pure_prompt_first_line
    ...
    set --local node_version (_pure_prompt_node_version)

    if test $pure_user_host_location -eq 0
        echo (_pure_print_prompt $current_folder $git_repo $user_and_host $node_version $command_duration)
    else
        echo (_pure_print_prompt $user_and_host $current_folder $git_repo $node_version $command_duration)
    end
end
